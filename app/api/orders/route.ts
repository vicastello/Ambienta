...
export async function GET(req: NextRequest) {
  try {
    ...
    const metricsParams: OrdersMetricsArgs = {
      p_data_inicial: dataInicial ?? undefined,
      p_data_final: dataFinal ?? undefined,
      p_canais: canais ?? undefined,
      p_situacoes: situacoes ?? undefined,
    ...
    const statusCounts: Record<string, number> = (metricsRow?.situacao_counts as Record<string, number>) ?? {};
