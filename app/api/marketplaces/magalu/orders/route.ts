...
export async function GET(req: Request) {
  ...
  if (!process.env.MAGALU_API_KEY_ID && !MAGALU_MOCK_MODE) {
    return NextResponse.json(
      {
        ok: false,
        error: {
          code: 'MAGALU_NOT_CONFIGURED',
          message: 'Magalu não configurado. Defina MAGALU_API_KEY_ID e MAGALU_API_KEY_SECRET nas variáveis de ambiente.',
        },
      },
      { status: 503 }
    );
